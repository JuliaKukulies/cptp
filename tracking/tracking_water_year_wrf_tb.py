...
from Tracking_Functions_modified import get_wrf_tb
...
DataName = 'WRF'
OutputFolder='/glade/scratch/kukulies/tracks/water-year/'  # you will have to modify this location
...
path = Path('/glade/scratch/kukulies/WY2020/nudging/')
...
for mon in np.arange(5,6): 
    month = str(mon).zfill(2)
    print('retrieving data for month',mon)
    data = xr.open_dataset(str(path) + '/olr_CPTP-WY2020-4_ECMWF-ERA5_evaluation_r2i1p1_BNU-WRF421P_v1_hour_'+str(year)+'-'+month+'_regridded.nc')
    # decrease domain size to Sichuan basin 
    DataAll = np.zeros(data.olr.shape) 
    for idx in np.arange(data.time.values.size):
        tt = data.time.values[idx]
        DataAll[idx] = np.array(get_tb(data.olr.sel(time = tt))) 
    Lat = data.lat.values
    Lon = data.lon.values
    ...
    grZGclonesPT = ObjectCharacteristics(objects_fin, # feature object file
                                    ...
                                    OutputFolder+'tbb/Cloud_tracking_water-year'+ str(year)+month +'_spectral-nudging_WRF',
    ...
    dataset = Dataset(OutputFolder+'tbb/Cloud_tracking_water-year_'+ str(year) + month+'_spectral-nudging_WRF.nc','w',format='NETCDF4_CLASSIC')
