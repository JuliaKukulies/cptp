...
# modified by Julia Kukulies 04 March, 2022 (julia.kukulies@gu.se)

# USER Input
CASE='4km_timelag' # ['4km_MCS', '4km_Monsoon', '4km_Snow']
VARS='pr' # ['pr','tas','swe','olr']
DataDir='/glade/scratch/kukulies/CPTP/wrfout/'
Institude='NCAR'
RCM='WRF'
Version='421'
Forcing='ECMWF-ERA5'
...
OutDIR='/glade/scratch/kukulies/CPTP/4km_timelag/postprocessed/'
if [ ! -d $OutDIR ]; then
  mkdir -p $OutDIR
  echo $OutDIR
fi
...
do 
	...
	if compgen -G "wrfout_*$YEAR-$MON*00"; then # check if any files for this year and month exist
		...
		do
		echo $fi
		ncks -O -v $wrfname $WRFOUT$fi $OutDIR$fi
		done
	    done
